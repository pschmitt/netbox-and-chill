 ...
 * horizontal swipe between [items] via [HorizontalPager], vertical drag-down dismisses (an
...
fun ImageViewerDialog(items: List<ImageViewerItem>, initialIndex: Int, onDismiss: () -> Unit) {
    ...
    Dialog(onDismissRequest = onDismiss, properties = DialogProperties(usePlatformDefaultWidth = false)) {
        ...
        Box(modifier = Modifier.fillMaxSize().background(Color.Black.copy(alpha = 1f - dismissProgress * 0.7f))) {
            Column(
                modifier =
                        ...
                        Modifier.fillMaxWidth().weight(1f).pointerInput(isZoomed) {
                            ...
                            detectVerticalDragGestures(
                                ...
                                onVerticalDrag = { change, dragAmount ->
                                    change.consume()
                                    scope.launch { dismissOffsetY.snapTo(dismissOffsetY.value + dragAmount) }
                                },
                            )
                        }
                ) {
                ...
                ImageMetadataPanel(item = items[pagerState.currentPage.coerceIn(0, items.lastIndex)])
            }
 ...
 * at 1x scale so the enclosing [HorizontalPager] (page swipe) and the dismiss-drag [Box] (vertical
 ...
 */
private suspend fun PointerInputScope.detectZoomPan(isZoomed: () -> Boolean, onGesture: (pan: Offset, zoom: Float) -> Unit) {
    awaitEachGesture {
...
private fun ImageMetadataPanel(item: ImageViewerItem) {
    Column(
        modifier =
            Modifier.fillMaxWidth().background(Color.Black.copy(alpha = 0.6f)).padding(horizontal = 16.dp, vertical = 12.dp)
    ) {
        Text(item.title, color = Color.White, style = MaterialTheme.typography.titleMedium)
        if (item.metadata.isNotEmpty()) {
            Spacer(Modifier.height(4.dp))
            item.metadata.forEach { (label, value) ->
                Text("$label: $value", color = Color.White.copy(alpha = 0.85f), style = MaterialTheme.typography.bodySmall)
            }
        }
    }
}
