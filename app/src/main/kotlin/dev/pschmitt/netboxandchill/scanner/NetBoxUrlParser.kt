...
object NetBoxUrlParser {
    ...
    fun parse(text: String): NetBoxTarget? {
        ...
        parseNbxc(trimmed)?.let { return it }
    ...
    /** Parses the app-owned `nbxc://` deep-link format. */
    private fun parseNbxc(text: String): NetBoxTarget? {
        val uri = runCatching { URI(text) }.getOrNull() ?: return null
        if (!uri.scheme.equals("nbxc", ignoreCase = true)) return null
