...
class NetBoxJsonTest {
    ...
    fun `reference accepts display and falls back to name`() {
        ...
        val withName = Json.parseToJsonElement("""{"manufacturer":{"id":3,"name":"Shelly"}}""") as JsonObject

        assertEquals(
            NetBoxJsonReference(7, "/api/dcim/devices/7/", "edge-7"),
            withDisplay.jsonReference("device"),
        )
        assertEquals(NetBoxJsonReference(3, display = "Shelly"), withName.jsonReference("manufacturer"))
    }

    @Test
    fun `common projections tolerate missing and non primitive fields`() {
        val value = Json.parseToJsonElement("""{"id":"bad","status":{"label":"Active"}}""") as JsonObject
